...
function renderPlanTable(plan) {
  ...
  return `
    <table class="plan-table">
      <thead>
        <tr>
          <th style="min-width:96px">Tag</th>
        ...
        ${plan.days.map(day => {
          ...
          return `<tr style="${rowStyle}">
            ...
            ${plan.employees.map(emp => {
              ...
              return `<td title="${escapeAttr(title)}"><div class="plan-cell ${display.className}" style="min-width:72px">${escapeHtml(display.text)}</div></td>`;
            }).join('')}
          </tr>`;
        }).join('')}
      </tbody>
...
function generatePlan(year, month, federalState) {
  ...
  context.weeks.forEach((week, weekIndex) => {
    ...
    if (saturdayInWeek) {
      ...
      let offSlots = Math.max(0, workingCandidates.length - 2);
    ...
    for (const emp of state.employees) {
      ...
      if (absentAllWeek || hasWeekFreeNow) continue;

      for (const day of planWeekdays) {
        const date = day.date;
        const unavailable = new Set(unavailableEmployees(date));
        if (unavailable.has(emp.id)) continue;
        if (employeeMap.get(emp.id).assignments[date] !== 'F' && employeeMap.get(emp.id).assignments[date] !== 'S' && employeeMap.get(emp.id).assignments[date] !== 'SO') continue;

        const availableEmployees = state.employees.filter(e => !unavailable.has(e.id));
        const currentFrCount = availableEmployees.filter(e => employeeMap.get(e.id).assignments[date] === 'FR').length;
        const minWorking = unavailable.size > 0 ? 3 : 4;

        if (currentFrCount > 0) continue;
        if ((availableEmployees.length - 1) < minWorking) continue;
