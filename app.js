  ...
  context.weeks.forEach((week, weekIndex) => {
    ...
    if (!isSplitWeek) {
      state.employees
        ...
        .forEach(emp => employeesExcludedFromAdditionalWeekdayFree.add(emp.id));
    }
    ...
    weekdays.forEach((day, dayIndex) => {
      ...
      let mandatoryFreeIds = state.employees
        .filter(emp => !unavailable.has(emp.id))
        .filter(emp => Number(emp.fixedFreeDay) === day.weekday && saturdayOffEmployeeId !== emp.id)
        .map(emp => emp.id);

      if (isSplitWeek && mandatoryFreeIds.length > 0) {
        const maxMandatory = Math.max(0, availableEmployees.length - 3);
        mandatoryFreeIds = mandatoryFreeIds.slice(0, maxMandatory);
      }
      ...
      if (remainingNeedIds.length > 0) {
        ...
        const currentDayInTargetMonth = isInTargetMonth(day.date, year, month);
        const futureOutsideTargetMonth = futureWeekdays.some(d => !isInTargetMonth(d.date, year, month));
        const mustUseExtraToday = remainingNeedIds.length >= futureWeekdays.length;
        const preferExtraToday = isSplitWeek
          ? (mustUseExtraToday && !futureOutsideTargetMonth)
          : (mustUseExtraToday || mandatoryFreeIds.length === 0);
      ...
      state.employees.forEach(emp => {
        if (freeIds.has(emp.id)) {
          ...
          employeeMap.get(emp.id).assignments[day.date] = absenceCodeFor(emp.id, day.date, holidays);
        }
      });
    });
    ...
    if (saturdayInWeek) {
      ...
      state.employees.forEach(emp => {
        ...
        if (saturdayOffEmployeeId === emp.id || fullVacationWeekIds.has(emp.id)) {
