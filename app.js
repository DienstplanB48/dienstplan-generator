...
function hasCompleteParkedWeek(weekDays, year, month) {
  const targetDays = weekDays.filter(d => isInTargetMonth(d.date, year, month));
  if (targetDays.length === 0) return false;
  return targetDays.every(d => state.employees.every(emp => state.parkedAssignments?.[d.date]?.[emp.id]));
}

function seedParkedAssignmentsIntoEmployeeMap(employeeMap, year, month) {
  Object.entries(state.parkedAssignments || {}).forEach(([date, entries]) => {
    if (!isInTargetMonth(date, year, month)) return;
    Object.entries(entries || {}).forEach(([employeeId, code]) => {
      if (employeeMap.has(employeeId)) {
        employeeMap.get(employeeId).assignments[date] = code;
      }
    });
  });
}

function storeParkedAssignment(parkedAssignments, date, employeeId, code, year, month) {
  if (isInTargetMonth(date, year, month)) return;
  parkedAssignments[date] = parkedAssignments[date] || {};
  parkedAssignments[date][employeeId] = code;
}
...
function generatePlan(year, month, federalState) {
  ...
  const employeeMap = new Map(
    state.employees.map(e => [
      e.id,
      {
        ...e,
        assignments: {},
        weeklyShiftPreference: null,
        saturdayOffCount: 0
      }
    ])
  );
  const parkedAssignments = structuredClone(state.parkedAssignments || {});
  seedParkedAssignmentsIntoEmployeeMap(employeeMap, year, month);
  ...
  context.weeks.forEach((week, weekIndex) => {
    ...
    if (hasCompleteParkedWeek(week.days, year, month)) {
      return;
    }

    let saturdayOffEmployeeId = null;
    if (saturdayInWeek) {
      const saturdayDate = saturdayInWeek.date;
      const saturdayUnavailable = new Set(unavailableEmployees(saturdayDate));
      const saturdayAvailableCount = state.employees.filter(emp => !saturdayUnavailable.has(emp.id) && !fullVacationWeekIds.has(emp.id)).length;
      const saturdayRequiredWorking = Math.max(1, state.employees.length - 2);
      const eligibleSaturday = state.employees.filter(emp => {
        if (!emp.saturdayRule) return false;
        if (emp.fixedFreeDay !== '' && emp.fixedFreeDay !== null && emp.fixedFreeDay !== undefined) return false;
        if (fullVacationWeekIds.has(emp.id)) return false;
        if (saturdayUnavailable.has(emp.id)) return false;
        return true;
      });

      if (eligibleSaturday.length > 0 && (saturdayAvailableCount - 1) >= saturdayRequiredWorking) {
        eligibleSaturday.sort((a, b) => {
          const countA = employeeMap.get(a.id).saturdayOffCount || 0;
          const countB = employeeMap.get(b.id).saturdayOffCount || 0;
          const orderA = state.employees.findIndex(x => x.id === a.id);
          const orderB = state.employees.findIndex(x => x.id === b.id);
          return countA - countB || orderA - orderB;
        });
        saturdayOffEmployeeId = eligibleSaturday[0].id;
        if (isInTargetMonth(saturdayDate, year, month)) {
    ...
    const employeesExcludedFromAdditionalWeekdayFree = new Set(
      state.employees
        .filter(emp => emp.fixedFreeDay !== '' && emp.fixedFreeDay !== null && emp.fixedFreeDay !== undefined)
        .map(emp => emp.id)
    );
    if (saturdayOffEmployeeId) employeesExcludedFromAdditionalWeekdayFree.add(saturdayOffEmployeeId);
    fullVacationWeekIds.forEach(id => employeesExcludedFromAdditionalWeekdayFree.add(id));
    ...
    weekdays.forEach((day, dayIndex) => {
      ...
      const mandatoryFreeIds = state.employees
        .filter(emp => !unavailable.has(emp.id))
        .filter(emp => Number(emp.fixedFreeDay) === day.weekday && saturdayOffEmployeeId !== emp.id)
        .map(emp => emp.id);

      mandatoryFreeIds.forEach(id => {
        weekdayFreeAssigned[id] = true;
      });
      ...
      if (remainingNeedIds.length > 0) {
        const extraCandidates = availableEmployees
          .filter(emp => !mandatoryFreeIds.includes(emp.id))
          .filter(emp => !employeesExcludedFromAdditionalWeekdayFree.has(emp.id))
          .filter(emp => !weekdayFreeAssigned[emp.id]);

        const mustUseExtraToday = remainingNeedIds.length >= futureWeekdays.length;
        const preferExtraToday = mustUseExtraToday || mandatoryFreeIds.length === 0;

        if (preferExtraToday && extraCandidates.length > 0) {
          extraCandidates.sort(
            (a, b) =>
              weeklyFreeScore(a.id, week.days, employeeMap, day.date) -
              weeklyFreeScore(b.id, week.days, employeeMap, day.date)
          );

          for (const candidate of extraCandidates) {
            const tentativeFreeIds = new Set([...mandatoryFreeIds, candidate.id]);
            const tentativeWorkingIds = availableEmployees.map(e => e.id).filter(id => !tentativeFreeIds.has(id));
            if (tentativeWorkingIds.length >= 3) {
              extraFreeEmployeeId = candidate.id;
              weekdayFreeAssigned[extraFreeEmployeeId] = true;
              break;
            }
          }
        }
      ...
      let freeIds = new Set(mandatoryFreeIds);
      let workingIds = availableEmployees.map(e => e.id).filter(id => !freeIds.has(id));

      if (extraFreeEmployeeId) {
        const tentativeFreeIds = new Set([...mandatoryFreeIds, extraFreeEmployeeId]);
        const tentativeWorkingIds = availableEmployees.map(e => e.id).filter(id => !tentativeFreeIds.has(id));
        if (tentativeWorkingIds.length >= 3) {
          freeIds = tentativeFreeIds;
          workingIds = tentativeWorkingIds;
        }
      }

      if (workingIds.length < 3) {
        throw new Error(`${formatDate(day.date)}: Zu wenige Mitarbeiter für diese Kalenderwoche. Feste freie Tage, Samstagregel oder Abwesenheiten kollidieren.`);
      }
      ...
      state.employees.forEach(emp => {
        if (freeIds.has(emp.id)) {
          employeeMap.get(emp.id).assignments[day.date] = 'FR';
          storeParkedAssignment(parkedAssignments, day.date, emp.id, 'FR', year, month);
        } else if (unavailable.has(emp.id)) {
          const code = absenceCodeFor(emp.id, day.date, holidays);
          employeeMap.get(emp.id).assignments[day.date] = code;
          storeParkedAssignment(parkedAssignments, day.date, emp.id, code, year, month);
        }
      });
    });
    ...
    if (saturdayInWeek) {
      ...
      state.employees.forEach(emp => {
        ...
        if (unavailable.has(emp.id)) {
          code = absenceCodeFor(emp.id, date, holidays);
        } else if (saturdayOffEmployeeId === emp.id || fullVacationWeekIds.has(emp.id)) {
          code = 'FR';
        } else {
          code = isSpecialDay(date, emp) ? 'SO' : 'F';
        }
        employeeMap.get(emp.id).assignments[date] = code;
        storeParkedAssignment(parkedAssignments, date, emp.id, code, year, month);
      });
    }

    const sunday = week.days.find(d => d.weekday === 6);
    if (sunday) {
      state.employees.forEach(emp => {
        employeeMap.get(emp.id).assignments[sunday.date] = 'FR';
        storeParkedAssignment(parkedAssignments, sunday.date, emp.id, 'FR', year, month);
      });
    }
  });
  ...
  state.parkedAssignments = parkedAssignments;
