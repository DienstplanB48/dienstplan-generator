...
function validatePlanInputs(year, month) {
  ...
  roster.weeks.forEach(week => {
    ...
    workDays.forEach(day => {
      ...
      if (availableCount < 3) warnings.push(`${formatDate(day.date)}: Weniger als 3 verfügbare Mitarbeiter laut Abwesenheiten.`);
    ...
    inTargetMonthDays.filter(d => d.weekday < 5).forEach(day => {
      ...
      if (availableEmployees.length < 3) {
        availableEmployees.forEach(emp => {
          employeeMap.get(emp.id).assignments[day.date] = 'F';
        });
