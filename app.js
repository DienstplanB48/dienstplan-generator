    ...
    weekdays.forEach((day, dayIndex) => {
      ...
      if (remainingNeedIds.length > 0) {
        ...
        if (preferExtraToday && extraCandidates.length > 0) {
          ...
          extraFreeEmployeeId = extraCandidates[0].id;
          weekdayFreeAssigned[extraFreeEmployeeId] = true;
        }
      }

      const freeIds = new Set([...mandatoryFreeIds, ...(extraFreeEmployeeId ? [extraFreeEmployeeId] : [])]);
      const workingIds = availableEmployees.map(e => e.id).filter(id => !freeIds.has(id));

      if (workingIds.length < 3) {
        throw new Error(`${formatDate(day.date)}: Zu wenige Mitarbeiter für diese Kalenderwoche. Feste freie Tage, Samstag-Regel oder Abwesenheiten kollidieren.`);
      }
