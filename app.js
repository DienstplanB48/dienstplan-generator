const STORAGE_KEY = 'dienstplan-generator-state-v2';
...
function sharePdf(id) {
  const plan = state.plans.find(p => p.id === id);
  if (!plan) return;
  const printable = `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(plan.label)}</title><link rel="stylesheet" href="styles.css"></head><body><main class="main-content">${renderPlanPreview(plan)}</main><script>window.onload=()=>window.print()</script></body></html>`;
...
function findPlanByMonth(year, month) {
  return state.plans.find(plan => {
    const match = /^(\D+)\s+(\d{4})$/.exec(plan.label || '');
    if (!match) return false;
    return Number(match[2]) === year && monthName(month) === match[1];
  }) || null;
}

function getCarryoverAssignments(year, month) {
  const prev = previousMonth(year, month);
  const prevPlan = findPlanByMonth(prev.year, prev.month);
  const carry = prevPlan?.meta?.carryoverNextWeek;
  if (!carry || !carry.assignmentsByDate) return { assignmentsByDate: {}, dates: [] };
  return carry;
}

function getExistingAssignmentForDate(employeeMap, employeeId, date) {
  return employeeMap.get(employeeId)?.assignments?.[date] || '';
}

function countExistingWeekFreeInWeek(employeeId, weekDays, employeeMap) {
  return weekDays.filter(d => getExistingAssignmentForDate(employeeMap, employeeId, d.date) === 'FR').length;
}

function countExistingFreeOnDate(date, employeeMap) {
  let count = 0;
  state.employees.forEach(emp => {
    if (getExistingAssignmentForDate(employeeMap, emp.id, date) === 'FR') count += 1;
  });
  return count;
}
...
function generatePlan(year, month, federalState) {
  ...
  const carryover = getCarryoverAssignments(year, month);
  ...
  state.employees.forEach(emp => {
    const carryAssignments = carryover.assignmentsByDate || {};
    Object.entries(carryAssignments).forEach(([date, byEmployee]) => {
      if (byEmployee && byEmployee[emp.id]) {
        employeeMap.get(emp.id).assignments[date] = byEmployee[emp.id];
      }
    });
  });
  ...
  context.weeks.forEach((week, weekIndex) => {
    ...
    const hasPreviousMonthDays = week.days.some(d => !isInTargetMonth(d.date, year, month) && new Date(d.date) < new Date(year, month, 1));
    const hasNextMonthDays = week.days.some(d => !isInTargetMonth(d.date, year, month) && new Date(d.date) > new Date(context.monthEnd));
    const preplannedDates = new Set(
      inTargetMonthDays
        .filter(d => state.employees.every(emp => !!getExistingAssignmentForDate(employeeMap, emp.id, d.date)))
        .map(d => d.date)
    );
    const workWeekdays = inTargetMonthDays.filter(d => d.weekday < 5 && !preplannedDates.has(d.date));
    const nonHolidayWeekdays = workWeekdays.filter(d => !holidays[d.date] && !isSpecialDate(d.date));
    const saturdayInWeek = week.days.find(d => d.weekday === 5 && isInTargetMonth(d.date, year, month) && !preplannedDates.has(d.date));
    const fullVacationWeekIds = new Set(
      state.employees.filter(e => hasVacationMonFriFullWeek(e.id, week.days)).map(e => e.id)
    );

    const saturdayOffIds = new Set();
    if (saturdayInWeek) {
      const date = saturdayInWeek.date;
      const unavailable = new Set(unavailableEmployees(date));
      const availableSaturday = state.employees.filter(e => !unavailable.has(e.id));
      if (availableSaturday.length < 3) {
        throw new Error(`${formatDate(date)}: Samstag nicht planbar, weniger als 3 Mitarbeiter verfügbar.`);
      }

      const existingSaturdayOffIds = state.employees
        .filter(emp => getExistingAssignmentForDate(employeeMap, emp.id, date) === 'FR')
        .map(emp => emp.id);
      existingSaturdayOffIds.forEach(id => saturdayOffIds.add(id));
      availableSaturday.forEach(emp => {
        if (fullVacationWeekIds.has(emp.id)) saturdayOffIds.add(emp.id);
      });

      const offSlots = Math.max(0, availableSaturday.length - 3);
      const orderedCandidates = availableSaturday
        .filter(emp => !saturdayOffIds.has(emp.id))
        .sort((a, b) => {
          const aPriority = a.saturdayRule ? 1 : 0;
          const bPriority = b.saturdayRule ? 1 : 0;
          if (bPriority !== aPriority) return bPriority - aPriority;

          const countA = employeeMap.get(a.id).saturdayOffCount || 0;
          const countB = employeeMap.get(b.id).saturdayOffCount || 0;
          if (countA !== countB) return countA - countB;

          const targetA = a.saturdayRule ? Math.abs(2 - countA) : 99;
          const targetB = b.saturdayRule ? Math.abs(2 - countB) : 99;
          if (targetA !== targetB) return targetA - targetB;

          return state.employees.findIndex(x => x.id === a.id) - state.employees.findIndex(x => x.id === b.id);
        });

      for (const emp of orderedCandidates) {
        if (saturdayOffIds.size >= offSlots) break;
        saturdayOffIds.add(emp.id);
        employeeMap.get(emp.id).saturdayOffCount += 1;
      }
    }

    const weekdayFreePlan = new Map();
    const employeesNeedingWeekdayFree = state.employees
      .filter(emp => !fullVacationWeekIds.has(emp.id))
      .filter(emp => !saturdayOffIds.has(emp.id))
      .filter(emp => countExistingWeekFreeInWeek(emp.id, week.days, employeeMap) === 0)
      .filter(emp => {
        if (!hasPreviousMonthDays) return true;
        const preferredDates = buildPreferredWeekdayFreeDates(emp, nonHolidayWeekdays, holidays);
        return preferredDates.length > 0;
      });

    for (const emp of employeesNeedingWeekdayFree) {
      const preferredDates = buildPreferredWeekdayFreeDates(emp, nonHolidayWeekdays, holidays);
      for (const date of preferredDates) {
        if (isAbsent(emp.id, date, ['vacation', 'wishfree', 'sick'])) continue;
        const unavailable = unavailableEmployees(date);
        const availableCount = state.employees.length - unavailable.length;
        const alreadyFreeCount = countExistingFreeOnDate(date, employeeMap) + [...weekdayFreePlan.values()].filter(x => x === date).length;
        const targetMinWorking = unavailable.length > 0 ? 3 : 4;
        if (alreadyFreeCount >= 1) continue;
        if (availableCount - (alreadyFreeCount + 1) < targetMinWorking) continue;
        weekdayFreePlan.set(emp.id, date);
        break;
      }
    }

    employeesNeedingWeekdayFree.forEach(emp => {
      if (!weekdayFreePlan.has(emp.id) && nonHolidayWeekdays.length > 0) {
        if (hasPreviousMonthDays) return;
        throw new Error(`${emp.name} hat in der KW ab ${formatDate(week.days[0].date)} keinen freien Wochentag erhalten.`);
      }
    });
    ...
    workWeekdays.forEach(day => {
      ...
      if (holidays[date] || isSpecialDate(date)) {
        state.employees.forEach(emp => {
          employeeMap.get(emp.id).assignments[date] = unavailable.has(emp.id)
            ? absenceCodeFor(emp.id, date, holidays)
            : 'H';
        });
        return;
      }
      ...
      if (availableEmployees.length < 3) {
        throw new Error(`${formatDate(date)}: Planung nicht möglich, weniger als 3 Mitarbeiter verfügbar.`);
      }
      ...
      [...weekdayFreePlan.entries()]
        .filter(([, freeDate]) => freeDate === date)
        .forEach(([id]) => freeIds.add(id));

      const workingIds = availableEmployees.map(e => e.id).filter(id => !freeIds.has(id));
      const targetMinWorking = unavailable.size > 0 ? 3 : 4;
      if (workingIds.length < targetMinWorking) {
        throw new Error(`${formatDate(date)}: Zu wenige Mitarbeiter nach Zuteilung des freien Tages.`);
      }
      ...
      availableEmployees.forEach(emp => {
        if (freeIds.has(emp.id)) employeeMap.get(emp.id).assignments[date] = 'FR';
      });
    ...
    if (saturdayInWeek) {
      ...
      const unavailable = new Set(unavailableEmployees(date));
      const availableSaturday = state.employees.filter(e => !unavailable.has(e.id));
      const workingSaturdayIds = availableSaturday
        .filter(emp => !saturdayOffIds.has(emp.id))
        .map(emp => emp.id);

      if (workingSaturdayIds.length !== 3) {
        throw new Error(`${formatDate(date)}: Samstag muss genau 3 arbeitende Mitarbeiter haben.`);
      }

      state.employees.forEach(emp => {
        if (unavailable.has(emp.id)) {
          employeeMap.get(emp.id).assignments[date] = absenceCodeFor(emp.id, date, holidays);
        } else if (saturdayOffIds.has(emp.id)) {
  ...
  const monthLastDate = formatIso(new Date(year, month, 0));
  const carryoverDates = context.weeks[context.weeks.length - 1].days
    .map(d => d.date)
    .filter(date => date > monthLastDate);
  const assignmentsByDate = {};
  carryoverDates.forEach(date => {
    assignmentsByDate[date] = {};
    state.employees.forEach(emp => {
      const code = employeeMap.get(emp.id).assignments[date];
      if (code) assignmentsByDate[date][emp.id] = code;
    });
  });
...
function buildPreferredWeekdayFreeDates(emp, nonHolidayWeekdays, holidays) {
  const dates = nonHolidayWeekdays.map(d => d.date);
  if (dates.length === 0) return [];

  if (emp.fixedFreeDay !== '' && emp.fixedFreeDay !== null && emp.fixedFreeDay !== undefined) {
    const fixed = Number(emp.fixedFreeDay);
    const exact = nonHolidayWeekdays.find(d => d.weekday === fixed && !holidays[d.date]);
    if (exact) {
      return [exact.date, ...dates.filter(date => date !== exact.date)];
    }

    const sorted = [...nonHolidayWeekdays].sort((a, b) => {
      const distA = Math.abs(a.weekday - fixed);
      const distB = Math.abs(b.weekday - fixed);
